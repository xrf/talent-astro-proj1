...
1003 format(1pe13.5,4e13.5)
     ...
     if (nc_stat == 2) then   !  file does not exist, so create it and define variables
        ...
        nc_stat = nf90_def_var(ncid, "Speed", nf90_float, DimIDs, speed_varID)
        ...
        nc_stat = nf90_inq_varid(ncid, "Speed", speed_varID)
     ...
     if(ndim==2) then
        nc_stat = nf90_put_var(ncid, speed_varID, sqrt(zux ** 2 + zuy ** 2), start, count)
     else
        nc_stat = nf90_put_var(ncid, speed_varID, sqrt(zux ** 2 + zuy ** 2 + zuz ** 2), start, count)
     endif
